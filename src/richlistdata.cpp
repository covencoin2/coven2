// Copyright (c) 2015 The coven developers
		      ...
		      if(!fInvalid)
		      {
			    ...
			    { 
			        ...
			        if(ExtractDestination(atxout.scriptPubKey, dest))
			        {
			    	        ...
			    	        std::string covenAddress = CBitcoinAddress(dest).ToString();
				        CRichListDataItem richListDataItem;
				        richListDataItem.sAddress = covenAddress;
				        if(richListData.mapRichList.find(covenAddress) == richListData.mapRichList.end())
				            richListData.mapRichList.insert(make_pair(covenAddress, richListDataItem));
					
				        richListData.mapRichList[covenAddress].nBalance = richListData.mapRichList[covenAddress].nBalance - atxout.nValue;
