...
static void NotifycovenNodeUpdated(MasternodeManager *page, CcovenNodeConfig nodeConfig)
{
    ...
    QMetaObject::invokeMethod(page, "updatecovenNode", Qt::QueuedConnection,
                              Q_ARG(QString, alias),
                              Q_ARG(QString, addr),
...
void MasternodeManager::subscribeToCoreSignals()
{
    ...
    uiInterface.NotifycovenNodeChanged.connect(boost::bind(&NotifycovenNodeUpdated, this, _1));
}

void MasternodeManager::unsubscribeFromCoreSignals()
{
    // Disconnect signals from core
    uiInterface.NotifycovenNodeChanged.disconnect(boost::bind(&NotifycovenNodeUpdated, this, _1));
}
...
void MasternodeManager::updatecovenNode(QString alias, QString addr, QString privkey, QString collateral)
{
    LOCK(cs_coven);
    bool bFound = false;
    int nodeRow = 0;
...
void MasternodeManager::updateNodeList()
{
    ...
    {
        LOCK(cs_coven);
        BOOST_FOREACH(PAIRTYPE(std::string, CcovenNodeConfig) coven, pwalletMain->mapMycovenNodes)
        {
            updatecovenNode(QString::fromStdString(coven.second.sAlias), QString::fromStdString(coven.second.sAddress), QString::fromStdString(coven.second.sMasternodePrivKey), QString::fromStdString(coven.second.sCollateralAddress));
        }
    }
}
...
{
    AddEditcovenNode* aenode = new AddEditcovenNode();
    aenode->exec();
}
...
void MasternodeManager::on_getConfigButton_clicked()
{
    ...
    int r = index.row();
    std::string sAddress = ui->tableWidget_2->item(r, 1)->text().toStdString();
    CcovenNodeConfig c = pwalletMain->mapMycovenNodes[sAddress];
    std::string sPrivKey = c.sMasternodePrivKey;
    covenNodeConfigDialog* d = new covenNodeConfigDialog(this, QString::fromStdString(sAddress), QString::fromStdString(sPrivKey));
    d->exec();
}
...
void MasternodeManager::on_removeButton_clicked()
{
    ...
    confirm = QMessageBox::question(this, "Delete coven Node?", "Are you sure you want to delete this coven node configuration?", QMessageBox::Yes|QMessageBox::No);
    ...
    if(confirm == QMessageBox::Yes)
    {
        ...
        pwalletMain->mapMycovenNodes.erase(sAddress);
        walletdb.ErasecovenNodeConfig(c.sAddress);
...
void MasternodeManager::on_startButton_clicked()
{
    ...
    QMessageBox msg;
    if(result)
        msg.setText("coven Node at " + QString::fromStdString(c.sAddress) + " started.");
    else
...
void MasternodeManager::on_stopButton_clicked()
{
    ...
    if(result)
    {
        msg.setText("coven Node at " + QString::fromStdString(c.sAddress) + " stopped.");
    }
    else
    {
        msg.setText("Error: " + QString::fromStdString(errorMessage));
...
void MasternodeManager::on_startAllButton_clicked()
{
    ...
    BOOST_FOREACH(PAIRTYPE(std::string, CcovenNodeConfig) coven, pwalletMain->mapMycovenNodes)
    {
        CcovenNodeConfig c = coven.second;
	std::string errorMessage;
        bool result = activeMasternode.RegisterByPubKey(c.sAddress, c.sMasternodePrivKey, c.sCollateralAddress, errorMessage);
	if(result)
	{
