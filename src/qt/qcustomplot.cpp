...
/*!  \internal
  ...
  plotting. Note that in order to get a clean plot all the way to the coven of the axis rect, \a
...
void QCPCurve::getCurveData(QVector<QPointF> *lineData) const
{
  ...
  while (it != mData->constEnd())
  {
    ...
    if (currentRegion != prevRegion) // changed region, possibly need to add some optimized coven points or original points if entering R
    {
