...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizecovenAmount"))
        settings.setValue("nAnonymizecovenAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizecovenAmount = settings.value("nAnonymizecovenAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizecovenAmount"))
        SoftSetArg("-anonymizecovenamount", settings.value("nAnonymizecovenAmount").toString().toStdString());
#ifdef USE_NATIVE_I2P
