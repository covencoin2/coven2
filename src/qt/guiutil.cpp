...
{
    // Convert coven:// to coven:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("coven://"))
    {
        uri.replace(0, 12, "coven:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "coven.lnk";
}
...
{
    return GetAutostartDir() / "coven.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("coven-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  coven-qt [" + tr("command-line options") + "]                     " + "\n";
