...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #covencoinTEST\r");
            Send(hSocket, "WHO #covencoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #covencoin\n");
            // randomly join #covencoin00-#covencoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #covencoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #covencoin%02d\r", channel_number).c_str());
        };
