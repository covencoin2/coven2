...
int nAnonymizecovenAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\coven
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\coven
    // Mac: ~/Library/Application Support/coven
    // Unix: ~/.coven
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "coven2";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "coven.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "covend.pid"));
